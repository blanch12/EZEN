...
<link rel="stylesheet" href="/resources/styles/goodslist.css">
...
<body>
	...
	<div class="container p-3 my-3">
		<h1>
			<i class="fa fa-align-justify"></i> 상품 리스트
		</h1>
		<form action="list.do" id="searchForm">
			...
			<div class="search-section">
					...
					<div class="input-group-prepend">
						<select class="form-control" id="cate_code1" name="cate_code1">
							...
							<c:forEach items="${listBig }" var="vo">
								<option value="${vo.cate_code1 }" <c:if test="${ goodsSearchVO.cate_code1 == vo.cate_code1}">selected</c:if>>${vo.cate_name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
					...
					<div class="input-group-prepend">
						<select class="form-control" id="cate_code2" name="cate_code2">
							...
							<c:forEach items="${listMid }" var="vo">
								<option value="${vo.cate_code2 }" <c:if test="${ goodsSearchVO.cate_code2 == vo.cate_code2}">selected</c:if>>${vo.cate_name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="input-group mb-3">
					<input type="text" class="form-control" placeholder="상품명 검색" id="goods_name" name="goods_name" value="${goodsSearchVO.goods_name }">
				</div>

				<div class="input-group mb-3">
					<input type="text" class="form-control" placeholder="최저가격입력" id="min_price" name="min_price" value="${goodsSearchVO.min_price }">
					<input type="text" class="form-control" placeholder="최고가격입력" id="max_price" name="max_price" value="${goodsSearchVO.max_price }">
				</div>
				...
				<c:forEach items="${list }" var="vo" varStatus="vs">
					...
					<div class="col-md-3 dataRow">
						...
						<div class="card-body title">
							<p class="card-text">
							<div>
								<strong style="border: 0.5px solid #E5E5E5; color: gray; font-size: 10px;">라이프스타일</strong>
							</div>
