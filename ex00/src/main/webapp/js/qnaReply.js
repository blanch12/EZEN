...
let qnaReplyService = {
	...
	"list": function(page, callback, error) {
		...
		$.getJSON( // (url, 성공함수, 실패함수)
			"/qnareply/list.do?page=" + page + "&no=" + no,
	...
	"write": function(reply, callback, error) {
		...
		$.ajax({
			type: "post", // 데이터 전송 방식
			url: "/qnareply/write.do", // mapping되는 url을 적는다.
	...
	"update": function(reply, callback, error) {
		...
		$.ajax({
			type : "post", // 데이터 전송 방식
			url : "/qnareply/update.do", // mapping url -> RestController에 mapping된
	...
	"delete": function(rno, callback, error) {
		...
		$.ajax({
			type : "get", // 데이터 전송 방식
			url : "/qnareply/delete.do?rno=" + rno,
			// 성공하는 함수
			success : function(result, status, xhr) {
				if (callback) callback(result);
				else alert(result);
