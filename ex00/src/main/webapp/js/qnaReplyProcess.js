/**
 * replyProcess.js 
 * qnaReplyService 객체를 이용한 댓글 처리 코드
 */
 
 // 만든이후 잘되는지 확인
 //qnaReplyService.list();
 
 function showList(page) {
 	// 데이터를 받아 옵니다.
 	qnaReplyService.list(page,
 		...
 		function (data) {
 			...
 			if (list == null || list.length == 0) {
 				...
 				$(".chat").html("<li>답변이 아직 등록되지 않았습니다.</li>");
 				return;
 			}
 			// 데이터가 있는 경우의 처리
 			// **** 댓글 리스트 출력 ****
 ...
 $(function() {
 	...
 	$("#replyUpdateBtn").click(function(){
 		...
 		// 댓글 수정 처리
 		qnaReplyService.update(reply, // 서버에 전달되는 데이터
